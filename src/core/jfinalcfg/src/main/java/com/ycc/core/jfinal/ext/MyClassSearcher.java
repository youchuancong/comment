...
public class MyClassSearcher {
    ...
    public static MyClassSearcher of(Class target) {
        return new MyClassSearcher(target);
    }
        ...
        if (!baseDir.exists() || !baseDir.isDirectory()) {
            ...
            for (int i = 0; i < filelist.length; i++) {
                ...
                if (readfile.isDirectory()) {
                    ...
                    if (MyClassSearcher.wildcardMatch(targetFileName, tempName)) {
                        String classname;
    ...
    public MyClassSearcher(Class target) {
    	this.target = target;
    }

    public MyClassSearcher injars(List<String> jars) {
        if (jars != null) {
    ...
    public MyClassSearcher inJars(String... jars) {
        if (jars != null) {
            for (String jar : jars) {
                includeJars.add(jar);
            }
    ...
    public MyClassSearcher classpath(String classpath) {
    ...
    public MyClassSearcher includeAllJarsInLib(boolean includeAllJarsInLib) {
        this.includeAllJarsInLib = includeAllJarsInLib;
        return this;
    }

    public MyClassSearcher libDir(String libDir) {
        this.libDir = libDir;
        return this;
    }

    public MyClassSearcher scanPackages(List<String> scanPaths) {
        if(scanPaths != null){
